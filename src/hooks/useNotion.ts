...
export const useNotion = (auth: string, tagDatabaseId: string) => {
  ...
  const fetchProperties = async (pageId: string, propertyIds: string[]): Promise<GetPagePropertyResponse[]> => {
  ...
  const getIcon = (icon: PageObjectResponse["icon"]): string | null => {
    if (!icon) {
      return null;
    }
  ...
  const getProperty = (response: GetPagePropertyResponse): Property | null => {
  ...
  const fetchTags = async (): Promise<readonly Tag[]> => {
  ...
  const getBookmark = (id: string, responses: GetPagePropertyResponse[], favicon: string | null): Bookmark => {
  ...
  const fetchDatabase = async (id: string): Promise<PageObjectResponse[]> => {
  ...
  const stockArticle = async (databaseId: string, articleProps: Article): Promise<Result<string>> => {
  ...
  const fetchBookmarks = async (databaseId: string): Promise<readonly Bookmark[]> => {
